...
const MapComponent = ({ userLocation, collectorLocation, pickupStatus }) => {
  ...
  const isValid = (coords) => coords && coords[0] !== 0 && coords[1] !== 0;
  ...
  useEffect(() => {
    ...
    try {
      ...
      map.current.on('load', () => {
        map.current.addSource('route', { 'type': 'geojson', 'data': { 'type': 'Feature', 'geometry': { 'type': 'LineString', 'coordinates': [] } } });
        map.current.addLayer({
          'id': 'route',
          'type': 'line',
          'source': 'route',
          'layout': { 'line-join': 'round', 'line-cap': 'round' },
          'paint': { 'line-color': '#0984e3', 'line-width': 6, 'line-opacity': 0.8 }
        });
      });
    } catch (err) {

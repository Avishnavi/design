...
const PickupWizard = ({ user, onComplete, onCancel }) => {
  ...
  useEffect(() => {
    const fetchDealers = async () => {
      if (step === 3 && location && wasteType) {
        try {
          setLoading(true);
          const res = await userAPI.getNearbyDealers(
  ...
  const categories = [
    ...
    { id: 'E-waste', name: 'E-waste', img: 'https://images.unsplash.com/photo-1550009158-9ebf69173e03?q=80&w=400&auto=format&fit=crop' },
    { id: 'Glass', name: 'Glass', img: 'https://cdn.pixabay.com/photo/2014/12/12/22/08/glass-565914_1280.jpg' },
    { id: 'Mixed Waste', name: 'Mixed Waste', img: 'https://images.unsplash.com/photo-1532996122724-e3c354a0b15b?q=80&w=400&auto=format&fit=crop' },
  ];
  ...
  const handleSubmit = async (dealerId) => {
    try {
      setLoading(true);
      await userAPI.createPickup({
        ...
        location: location,
        assignedDealer: dealerId, // Send the selected dealer ID
        area: user?.area,
              ...
              <div key={dealer._id} className="dealer-card">
                <div className="dealer-info">
                  <div className="dealer-header-row">
                    <h4>{dealer.name}</h4>
                    {dealer.matchType && (
                      <span className={`match-badge ${dealer.matchType.replace(' ', '-').toLowerCase()}`}>
                        {dealer.matchType}
                      </span>
                    )}
                  </div>
                ...
                <div className="dealer-action">
                  ...
                  <button className="btn-select" onClick={() => handleSubmit(dealer._id)} disabled={loading}>{loading ? '...' : 'Select'}</button>
                </div>
              </div>
            ))}
            {dealers.length === 0 && !loading && <p>No dealers found for this material in your area.</p>}
          </div>
        </div>
      )}

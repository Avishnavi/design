...
const CollectorDashboard = () => {
  ...
  const [view, setView] = useState('dashboard'); // 'dashboard', 'history', 'map', 'profile'
  const [collectorLocation, setCollectorLocation] = useState([77.5946, 12.9716]); // Default [lng, lat]
  const [profileData, setProfileData] = useState({ name: '', phone: '', address: '', area: '', district: '' });
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // Load profile info into state for editing
      const user = dashRes.data.data.user; // Assuming dashboard returns user info
      if (user) {
        setProfileData({
          name: user.name || '',
          phone: user.phone || '',
  ...
  const handleProfileUpdate = async (e) => {
    ...
    try {
      setLoading(true);
      await collectorAPI.updateProfile(profileData);
          ...
          <div className="navigator-details-card">
            ...
            <div className="quick-stats-row">
              ...
              <div className="q-stat">
                ...
                <span className="q-value">₹ {activePickup.agreedPrice || (activePickup.quantity || 0) * 12}</span>
              </div>
            </div>
            ...
            <div className="phase-control-box manual">
               ...
               <div className="manual-update-section">
                 ...
                 >
                   {['Assigned', 'On The Way', 'Arrived', 'Collected', 'SentToDealer'].map((s, idx, arr) => {
                     const statusSequence = ['Pending', 'Assigned', 'On The Way', 'Arrived', 'Collected', 'SentToDealer', 'Recycled'];
                     const currentIndex = statusSequence.indexOf(activePickup.status);
                     const optionIndex = statusSequence.indexOf(s);
                     
                     // Only show current status and future statuses
  ...
  if (view === 'profile') {
    return (
      <div className="collector-profile container fade-in">
        <header className="module-header-new">
          <div className="header-left-group">
            <button className="btn-back-circle" onClick={() => setView('dashboard')}>✕</button>
            <div className="header-title-box">
              <h2>My Profile</h2>
              <p>Manage your account and operational area</p>
            </div>
          </div>
        </header>

        <form className="profile-form-new card" onSubmit={handleProfileUpdate}>
          <div className="form-sections-grid">
            ...
            <div className="form-section">
              ...
              <div className="form-row">
                <div className="form-group">
                  <label>Operation Area (e.g. HSR)</label>
                  <input 
                    type="text" 
    ...
    <div className="collector-dashboard container">
      <header className="dashboard-header-new">
        ...
        <div className="header-actions-row">
          <button className="btn-profile-link" onClick={() => setView('profile')}>
            👤 Profile
          </button>
                ...
                <div key={pickup._id} className="pickup-item-card">
                    <div className="pickup-main">
                      ...
                      <div className="pickup-tags">
                        ...
                        <span className="tag price">₹ {pickup.agreedPrice || 'N/A'}</span>
                        <span className={`tag status-chip ${pickup.status.toLowerCase()}`}>{pickup.status}</span>
                      </div>
                      {pickup.assignedDealer && (
                        <div className="assigned-dealer-info">
                          <span className="dealer-icon">🏢</span>
                          <span className="dealer-text">Deliver to: <strong>{pickup.assignedDealer.dealerName}</strong> ({pickup.assignedDealer.area || pickup.assignedDealer.district})</span>
                        </div>
                      )}
                    </div>
